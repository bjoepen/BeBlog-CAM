...
if "SpindleHeadGeometry" not in s:
    s=once(s,"  import type { MachineEnvelope, MachineWcsOrigin } from './machineEnvelope';\n","  import type { MachineEnvelope, MachineWcsOrigin } from './machineEnvelope';\n  import type { SpindleHeadGeometry } from './spindleHeadCollision';\n",'gcode import')
if 'export let spindleHead:' not in s:
...
if '<SpindleHeadSetupPanel' not in s:
    start=s.find('<MachineSetupPanel ')
    if start<0: raise SystemExit('MachineSetupPanel tag missing')
    end=s.find('/>',start)
    if end<0: raise SystemExit('MachineSetupPanel close missing')
    end+=2
    tag="<SpindleHeadSetupPanel enabled={spindleHeadEnabled} geometry={spindleHead} onEnabledChange={value=>spindleHeadEnabled=value} onGeometryChange={value=>spindleHead=value}/>"
    s=s[:end]+tag+s[end:]
