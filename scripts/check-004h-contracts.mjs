...
assert(app.includes("updateContour({depthMode:'manual'})")&&app.includes("updateContour({depthMode:'stock-bottom'})"),'Bearbeiten inspector exposes manual and through-cut modes');
assert(app.includes('updateContourOvercut')&&app.includes('End-Z'),'Bearbeiten inspector exposes overcut and resolved end Z');
assert(app.includes("!(operation.kind==='contour'&&(operation.depthMode??'manual')==='stock-bottom')"),'manual depth input is hidden while through-cut owns the target depth');
