<script lang="ts">
  ...
  function curvePoints(curve:Curve2){
    ...
    if(curve.kind==='arc'){let a=curve.startAngleDeg,b=curve.endAngleDeg;while(b<a)b+=360;return Array.from({length:65},(_,i)=>{const r=(a+(b-a)*i/64)*Math.PI/180;return{x:curve.center.x+Math.cos(r)*curve.radius,y:curve.center.y+Math.sin(r)*curve.radius}})}return[];
  }
  ...
  <main class="workspace">
    ...
    <aside class="inspector">
      ...
      {:else if activeStep==='Rohling'}
        ...
        {:else}<p class="note"><strong>Kein Rohling.</strong> Die DXF wird direkt als planare Bauteilgeometrie verwendet.</p>{/if}
