...
{:else}
  <div class="release fail"><strong>FAIL</strong><span>Carve-G-Code wird nur aus dem in Gate 6C freigegebenen Centerline-Pfad erzeugt.</span></div>{#each result.errors as error}<p class="error-line"><strong>FAIL</strong> {error}</p>{/each}{#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
<p class="note"><strong>001H Gate 6D:</strong> Der bewiesene Carve-Pfad bleibt unverändert; 001I verknüpft ihn nur im Gesamtjob.</p>
{/if}
