...
{#if result.ok}
  ...
  <div class="facts"><span>{result.segmentCount} Segmente</span><span>{result.passes} Zustellung{result.passes===1?'':'en'}</span><span>{result.totalCenterlineLengthMm.toFixed(3)} mm Centerline</span><span>{result.lineCount} G-Code-Zeilen</span></div>
  <p class="note"><strong>Strategie:</strong> Keine Radiuskorrektur. Jedes Segment wird auf Sicherheits-Z angefahren, auf Solltiefe eingetaucht und exakt entlang der DXF-Linie abgefahren. Die Segmentreihenfolge wird nur für kurze sichere Leerwege optimiert; einzelne Linien dürfen dafür lediglich umgekehrt durchlaufen werden.</p>
  {#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
  <div class="export-box"><div><strong>NC-Datei</strong><span>Speichert exakt den aktuell angezeigten Carve-G-Code als .nc.</span></div><button class="primary-export" onclick={saveNc}>G-Code speichern …</button></div>
...
{:else}
  <div class="release fail"><strong>FAIL</strong><span>Carve-G-Code wird nur aus dem in Gate 6C freigegebenen Centerline-Pfad erzeugt.</span></div>
  {#each result.errors as error}<p class="error-line"><strong>FAIL</strong> {error}</p>{/each}{#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
<p class="note"><strong>001H Gate 6D:</strong> Zunächst nur exakte offene DXF-Linien. ARC und offene Polylinien bleiben bis zu einem eigenen Interpolations-Gate gesperrt.</p>
