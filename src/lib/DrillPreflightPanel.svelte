<script lang="ts">
  ...
  import { resolvedDxfDrillDepth, validateDrillOperation } from './drillGcode';
  ...
  $: dxfDepthState=summary.kind==='dxf'?resolvedDxfDrillDepth(operation,stock,stockMode):null;
  ...
  $: warnings=summary.kind==='dxf'
    ?[...(dxfValidation?.warnings??[]),...(stockMode==='none'?['Kein Rohling definiert: Materialgrenzen können nur eingeschränkt geprüft werden.']:[]),...((operation.depthMode??'manual')==='manual'&&stockMode!=='none'&&(dxfDepthState?.depthMm??0)>stock.thickness?[`Bohrtiefe ${(dxfDepthState?.depthMm??0).toFixed(3)} mm überschreitet die Rohlingdicke ${stock.thickness.toFixed(3)} mm.`]:[])]
    :[
  ...
  $: errors=summary.kind==='dxf'
    ?[...(dxfValidation?.errors??[]),...(dxfDepthState&&!dxfDepthState.ok&&dxfDepthState.error?[dxfDepthState.error]:[]),...(wcs.z!=='top'?['WCS Unterseite ist für Bohren und Helixfräsen nicht freigegeben.']:[])]
    :[
  ...
  $: targetDepth=summary.kind==='dxf'
    ?(dxfDepthState?.ok?dxfDepthState.depthMm:0)
    :(operation.depthMode??'manual')==='stock-bottom'
      ?stock.thickness+(operation.overcutMm??0)
      :operation.totalDepthMm>0?operation.totalDepthMm:maxStepDepth;
  $: passes=targetDepth>0?Math.max(1,Math.ceil(targetDepth/Math.max(operation.stepDownMm,1e-9))):0;
...
<div class="checks"><div><strong>Geometrie</strong><p>{summary.kind==='step'?'Sicher erkannte zylindrische STEP/BRep-Hole-Features liefern Mittelpunkt, Achse, Durchmesser und Tiefe.':'Jeder ausgewählte native DXF-Kreis definiert Bohrungsmittelpunkt und Soll-Durchmesser.'}</p></div><div><strong>Herstellungsverfahren</strong><p>{operation.method==='helical-mill'?'Schaftfräser auf Fräsermittelbahnradius (Bohrungs-Ø − Werkzeug-Ø) / 2; dieselbe kanonische G3-Halbkreis-/Pitch-Primitive wird für DXF und STEP verwendet, anschließend ein Fertigumlauf auf Endtiefe.':'Explizite G0/G1-Bewegungen: Safe-Z → XY → axial bohren → Rückzug. Keine controllerabhängigen Canned Cycles.'}</p></div><div><strong>Tiefe</strong><p>{(operation.depthMode??'manual')==='stock-bottom'?`Durch Rohling: ${stock.thickness.toFixed(3)} mm + ${(operation.overcutMm??0).toFixed(3)} mm Overcut → ${targetDepth.toFixed(3)} mm Gesamttiefe.`:summary.kind==='step'?`${operation.totalDepthMm.toFixed(3)} mm konfigurierte Gesamttiefe; erkannte STEP-Geometrie dient als Plausibilitätsgrenze.`:`${operation.totalDepthMm.toFixed(3)} mm Gesamttiefe · max. ${operation.stepDownMm.toFixed(3)} mm je Zustellung.`}</p></div></div>
