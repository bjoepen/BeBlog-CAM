...
export function generateContourGcode(args:{summary:ImportSummary;stock:StockDefinition;stockMode:StockMode;placement:PartPlacement;orientation:PartOrientation;wcs:WorkCoordinateSystem;operation:ContourOperation}):GcodeResult{
  ...
  if(summary.kind!=='dxf')errors.push('001F erzeugt G-Code zunächst nur aus DXF-Konturen.');
  ...
  if(wcs.z!=='top')errors.push('G-Code mit WCS auf Unterseite ist in 001F noch gesperrt.');
  ...
  if(errors.length||!t)return{ok:false,errors,warnings,code:'',lineCount:0,pointCount:0,passes:0,radiusMm:operation.tool.diameterMm/2};
  ...
  if(!selected)return{ok:false,errors:['Gewählte Kontur wurde in der aktuellen Geometrie nicht gefunden.'],warnings,code:'',lineCount:0,pointCount:0,passes:0,radiusMm:operation.tool.diameterMm/2};
  ...
  if(!validation.ok)return{ok:false,errors:[`Werkzeugbahn hat die geometrische Prüfung nicht bestanden (max. Abweichung ${Number.isFinite(validation.maxDeviationMm)?validation.maxDeviationMm.toFixed(4):'—'} mm).`],warnings,code:'',lineCount:0,pointCount:toolpath.length,passes:0,radiusMm:radius,validation};
  ...
  lines.push(`G0 Z${f3(operation.safeZMm)}`);
  lines.push('M30');
  return{ok:true,errors,warnings,code:lines.join('\n'),lineCount:lines.length,pointCount:toolpath.length,passes,radiusMm:radius,validation};
}
