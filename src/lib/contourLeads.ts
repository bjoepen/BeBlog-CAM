...
export function resolveContourLeads(operation:ContourOperation):ContourLeadResolution{
  ...
  if(operation.topology!=='closed')errors.push('Tangentiale Ein-/Ausfahrten sind nur für geschlossene Konturen freigegeben; offene Nuten verwenden 004Z-E2 Rampeneinfahrt.');
...
function applyLeadToRun(run:CanonicalToolpathRun,cfg:ContourLeadResolution,safeZMm:number){
  ...
  run.entrySegments=[vertical(leadStart,safeZMm,run.z),horizontal(leadStart,first,run.z)];
  if(cfg.leadOutMm>EPS){
    const leadEnd={x:last.x+dOut.x*cfg.leadOutMm,y:last.y+dOut.y*cfg.leadOutMm};
    run.exitSegments=[horizontal(last,leadEnd,run.z),vertical(leadEnd,run.z,safeZMm)];
  }
  return true;
}
...
export function applyContourLeads(toolpath:CanonicalToolpath,operation:ContourOperation):{toolpath:CanonicalToolpath;errors:string[];warnings:string[]}{
  ...
  if(!applied)return{toolpath,errors:['Für Lead-in/Lead-out wurde keine zusammenhängende geschlossene Konturpassage gefunden.'],warnings:cfg.warnings};
  return{toolpath:{...toolpath,runs},errors:[],warnings:[...cfg.warnings,`Tangentiale Leads aktiv (geschlossene Kontur): Einfahrt ${cfg.leadInMm.toFixed(3)} mm · Ausfahrt ${cfg.leadOutMm.toFixed(3)} mm.`]};
}
