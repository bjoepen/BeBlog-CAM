<script lang="ts">
  ...
  import type { Curve2, ImportSummary, Point2, StockDefinition, PartPlacement } from './types';
  export let summary: ImportSummary;
  export let stock: StockDefinition;
  export let placement: PartPlacement;

  type P2={x:number;y:number}; type P3={x:number;y:number;z:number}; type View={yaw:number;pitch:number};
  ...
  function bounds3(points:P3[]){const xs=points.map(p=>p.x),ys=points.map(p=>p.y),zs=points.map(p=>p.z);return{minX:Math.min(...xs),maxX:Math.max(...xs),minY:Math.min(...ys),maxY:Math.max(...ys),minZ:Math.min(...zs),maxZ:Math.max(...zs)};}
  function bounds2(points:P2[]){const xs=points.map(p=>p.x),ys=points.map(p=>p.y);return{minX:Math.min(...xs),maxX:Math.max(...xs),minY:Math.min(...ys),maxY:Math.max(...ys)};}
  function placementXY(minX:number,maxX:number,minY:number,maxY:number){const pw=maxX-minX,ph=maxY-minY;const targetX=placement.horizontal==='left'?0:placement.horizontal==='right'?stock.width-pw:(stock.width-pw)/2;const targetY=placement.vertical==='front'?0:placement.vertical==='back'?stock.height-ph:(stock.height-ph)/2;return{dx:targetX-minX+placement.offsetX,dy:targetY-minY+placement.offsetY};}
  ...
  function scene3d(v:View){const values=summary.brep?.displayVertices??[],raw:P3[]=[];for(let i=0;i+2<values.length;i+=3)raw.push({x:values[i],y:values[i+1],z:values[i+2]});if(!raw.length)return null;const b=bounds3(raw),xy=placementXY(b.minX,b.maxX,b.minY,b.maxY),dz=-b.minZ+placement.offsetZ,part=raw.map(p=>({x:p.x+xy.dx,y:p.y+xy.dy,z:p.z+dz}));const m=Math.max(stock.width,stock.height)*.12+10,plane:P3[]=[{x:-m,y:-m,z:0},{x:stock.width+m,y:-m,z:0},{x:stock.width+m,y:stock.height+m,z:0},{x:-m,y:stock.height+m,z:0}],s=stock,box:P3[]=[{x:0,y:0,z:0},{x:s.width,y:0,z:0},{x:s.width,y:s.height,z:0},{x:0,y:s.height,z:0},{x:0,y:0,z:s.thickness},{x:s.width,y:0,z:s.thickness},{x:s.width,y:s.height,z:s.thickness},{x:0,y:s.height,z:s.thickness}],al=Math.max(35,Math.min(s.width,s.height)*.55),axes:P3[]=[{x:0,y:0,z:0},{x:al,y:0,z:0},{x:0,y:0,z:0},{x:0,y:al,z:0},{x:0,y:0,z:0},{x:0,y:0,z:al}];const pp=part.map(q=>project(q,v)),pl=plane.map(q=>project(q,v)),pb=box.map(q=>project(q,v)),pa=axes.map(q=>project(q,v)),map=fit([...pp,...pl,...pb,...pa]),fp=pp.map(map),fpl=pl.map(map),fb=pb.map(map),fa=pa.map(map);let partPath='';for(let i=0;i+2<fp.length;i+=3)partPath+=path([fp[i],fp[i+1],fp[i+2]],true)+' ';const edges=[[0,1],[1,2],[2,3],[3,0],[4,5],[5,6],[6,7],[7,4],[0,4],[1,5],[2,6],[3,7]];return{partPath,planePath:path(fpl,true),stockPaths:edges.map(([a,b])=>path([fb[a],fb[b]])),axes:[path([fa[0],fa[1]]),path([fa[2],fa[3]]),path([fa[4],fa[5]])],labels:[fa[1],fa[3],fa[5]]};}

  function scene2d(){const curves=summary.planarGeometry?.curves??[],samples=curves.map(sample),flat=samples.flat();if(!flat.length)return null;const b=bounds2(flat),xy=placementXY(b.minX,b.maxX,b.minY,b.maxY),placed=samples.map(a=>a.map(p=>({x:p.x+xy.dx,y:p.y+xy.dy}))),m=Math.max(stock.width,stock.height)*.12+10,plane=[{x:-m,y:-m},{x:stock.width+m,y:-m},{x:stock.width+m,y:stock.height+m},{x:-m,y:stock.height+m}],box=[{x:0,y:0},{x:stock.width,y:0},{x:stock.width,y:stock.height},{x:0,y:stock.height}],map=fit([...placed.flat(),...plane]),mapped=placed.map(a=>a.map(map)),fp=plane.map(map),fb=box.map(map),o=map({x:0,y:0}),al=Math.max(30,Math.min(stock.width,stock.height)*.45),xe=map({x:al,y:0}),ye=map({x:0,y:al});return{paths:mapped.map((p,i)=>path(p,curves[i]?.kind==='circle'||(curves[i]?.kind==='polyline'&&curves[i].closed))).filter(Boolean),plane:path(fp,true),stock:path(fb,true),axes:[path([o,xe]),path([o,ye])],labels:[xe,ye]};}
  ...
  $: s3=scene3d({yaw,pitch}); $: s2=scene2d();
  $: cameraTransform=`translate(${panX} ${panY}) translate(${width/2} ${height/2}) scale(${zoom}) translate(${-width/2} ${-height/2})`;
</script>
...
<div class="geometry-view">
  <svg bind:this={viewport} viewBox={`0 0 ${width} ${height}`} class:interactive={summary.kind==='step'}>
    {#if summary.kind==='dxf' && s2}
      <path d={s2.plane} class="setup-plane"/><path d={s2.stock} class="stock"/><path d={s2.axes[0]} class="axis x"/><path d={s2.axes[1]} class="axis y"/><text x={s2.labels[0].x+7} y={s2.labels[0].y-5}>X</text><text x={s2.labels[1].x+7} y={s2.labels[1].y-5}>Y</text>{#each s2.paths as p}<path d={p} class="dxf"/>{/each}
    {:else if s3}
      <g transform={cameraTransform}>
        <path d={s3.planePath} class="setup-plane"/>{#each s3.stockPaths as p}<path d={p} class="stock"/>{/each}<path d={s3.axes[0]} class="axis x"/><path d={s3.axes[1]} class="axis y"/><path d={s3.axes[2]} class="axis z"/><text x={s3.labels[0].x+7} y={s3.labels[0].y-5}>X</text><text x={s3.labels[1].x+7} y={s3.labels[1].y-5}>Y</text><text x={s3.labels[2].x+7} y={s3.labels[2].y-5}>Z</text><path d={s3.partPath} class="step"/>
      </g>
    {/if}
  </svg>
