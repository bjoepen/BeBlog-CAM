<script lang="ts">
  ...
  function fit(points:P2[],z=1,px=0,py=0){const p=points.filter(finite);if(!p.length)return(q:P2)=>q;const xs=p.map(q=>q.x),ys=p.map(q=>q.y),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),sx=Math.max(maxX-minX,1e-9),sy=Math.max(maxY-minY,1e-9),scale=Math.min((width-2*pad)/sx,(height-2*pad)/sy)*z,uw=sx*scale,uh=sy*scale,ox=(width-uw)/2+px,oy=(height-uh)/2+py;return(q:P2)=>({x:ox+(q.x-minX)*scale,y:height-(oy+(q.y-minY)*scale)});}
