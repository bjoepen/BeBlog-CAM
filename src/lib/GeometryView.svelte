<script lang="ts">
  ...
  function scene3d(v:View,includeZLevels:boolean,sliceStepMm:number,faceSelection:number[],allowanceMm:number,toolDiameterMm:number,stepoverPercent:number,jobToolpaths:CanonicalToolpath[]=[],includeModelRegions=false,modelSliceStepMm=2,includeRoughingRegions=false,includeModelToolpath=false,includeCurvedFaceProof=false,includeCurvedFaceRoughing=false,includeBallnoseContactProof=false){
    ...
    const map=fit([...pp,...ep.flatMap(edge=>edge.points),...rr.flat(2),...tp.flat(),...mr.flat(),...mi.flat(),...rg.flat(),...ri.flat(),...mt.flat(),...cf.flat(),...scp.flat(),...cr.flat(),...bn.flatMap(item=>[item.surface,item.center,item.normalEnd]),...pl,...pb,...pa]),fpl=pl.map(map),fb=pb.map(map),fa=pa.map(map),fw=map(pw);
...
<div class="geometry-view" bind:this={root}>
  <svg bind:this={viewport} viewBox="0 0 1000 650" class:interactive={summary.kind==='step'||(summary.kind==='dxf'&&drillViewMode==='25d')} onclick={pickVisibleFace}>
    ...
    {:else if s3}
      ...
      {#if showZLevels||preflightFaceTargetToolpaths.length||preflightStepToolpaths.length||canonicalToolpath?.operationKind==='surface-finishing'||canonicalToolpath?.operationKind==='drill'}{#if showZLevels}{#each s3.roughRegions as region}<path d={region} class="roughing-region" fill-rule="evenodd"/>{/each}{/if}{#each s3.toolPaths as tool}<path d={tool} class="toolpath-preview"/>{/each}{/if}
