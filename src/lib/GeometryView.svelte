<script lang="ts">
  ...
  function stockTriangles():P3[]{const w=stock.width,h=stock.height,t=stock.thickness,v=[{x:0,y:0,z:0},{x:w,y:0,z:0},{x:w,y:h,z:0},{x:0,y:h,z:0},{x:0,y:0,z:t},{x:w,y:0,z:t},{x:w,y:h,z:t},{x:0,y:h,z:t}],f=[[0,1,2],[0,2,3],[4,6,5],[4,7,6],[0,4,5],[0,5,1],[1,5,6],[1,6,2],[2,6,7],[2,7,3],[3,7,4],[3,4,0]];return f.flatMap(([a,b,c])=>[v[a],v[b],v[c]])}
  function pointInPolygon(p:P2,poly:P2[]){let inside=false;for(let i=0,j=poly.length-1;i<poly.length;j=i++){const a=poly[i],b=poly[j],hit=((a.y>p.y)!==(b.y>p.y))&&(p.x<(b.x-a.x)*(p.y-a.y)/(b.y-a.y||1e-12)+a.x);if(hit)inside=!inside}return inside}
  function pointInPartAtLevel(p:P2,chains:{points:P2[];closed:boolean}[]){let hits=0;for(const chain of chains)if(chain.closed&&chain.points.length>=4&&pointInPolygon(p,chain.points))hits++;return hits%2===1}
  function roughingSegments(stockChain:P2[],partChains:{points:P2[];closed:boolean}[],z:number):P3[][]{if(stockChain.length<4)return[];const segments:P3[][]=[];for(let i=0;i<stockChain.length-1;i++){const a=stockChain[i],b=stockChain[i+1],steps=Math.max(1,Math.ceil(Math.hypot(b.x-a.x,b.y-a.y)/3));let run:P3[]=[];for(let k=0;k<=steps;k++){const q={x:a.x+(b.x-a.x)*k/steps,y:a.y+(b.y-a.y)*k/steps},outside=!pointInPartAtLevel(q,partChains);if(outside)run.push({...q,z});else if(run.length){segments.push(run);run=[]}}if(run.length)segments.push(run)}return segments}

  function scene3d(v:View,includeZLevels:boolean,sliceStepMm:number,faceSelection:number[]){
    const a=summary.brep?.displayVertices??[],raw:P3[]=[];for(let i=0;i+2<a.length;i+=3)raw.push(rotate3({x:a[i],y:a[i+1],z:a[i+2]}));if(!raw.length)return null;
    const b=bounds3(raw),p=place(b.minX,b.maxX,b.minY,b.maxY),place3=(q:P3)=>({x:q.x+p.dx,y:q.y+p.dy,z:q.z-b.minZ+placement.offsetZ}),part=raw.map(place3),faceIds=summary.brep?.displayFaceIds??[];
    ...
    const modelSlices=includeZLevels?sliceTrianglesByStep(part,Math.max(.1,sliceStepMm)):[],stockMesh=stockTriangles(),roughWorld:P3[][]=[];
    if(includeZLevels){const stockSlices=sliceTrianglesByStep(stockMesh,Math.max(.1,sliceStepMm));for(const slice of modelSlices){const stockSlice=stockSlices.find(s=>Math.abs(s.z-slice.z)<1e-5);const stockOuter=stockSlice?.chains.find(c=>c.closed)?.points??[];roughWorld.push(...roughingSegments(stockOuter,slice.chains,slice.z))}}
    const m=Math.max(stock.width,stock.height)*.12+10,plane:P3[]=[{x:-m,y:-m,z:0},{x:stock.width+m,y:-m,z:0},{x:stock.width+m,y:stock.height+m,z:0},{x:-m,y:stock.height+m,z:0}],box:P3[]=[{x:0,y:0,z:0},{x:stock.width,y:0,z:0},{x:stock.width,y:stock.height,z:0},{x:0,y:stock.height,z:0},{x:0,y:0,z:stock.thickness},{x:stock.width,y:0,z:stock.thickness},{x:stock.width,y:stock.height,z:stock.thickness},{x:0,y:stock.height,z:stock.thickness}],wp=wcsPoint(),al=Math.max(35,Math.min(stock.width,stock.height)*.55),axes=[wp,{x:wp.x+al,y:wp.y,z:wp.z},wp,{x:wp.x,y:wp.y+al,z:wp.z},wp,{x:wp.x,y:wp.y,z:wp.z+al}];
    const pp=part.map(q=>projectPoint(q,v)),ep=edgeWorld.map(edge=>edge.map(q=>projectPoint(q,v))),rp=roughWorld.map(chain=>chain.map(q=>projectPoint(q,v))),pl=plane.map(q=>projectPoint(q,v)),pb=box.map(q=>projectPoint(q,v)),pa=axes.map(q=>projectPoint(q,v)),pw=projectPoint(wp,v),map=fit([...pp,...ep.flat(),...rp.flat(),...pl,...pb,...pa]),fpl=pl.map(map),fb=pb.map(map),fa=pa.map(map),fw=map(pw),triangles=projectTriangles(part,v,map,faceIds),edges=ep.map(edge=>path(edge.map(map))).filter(Boolean),roughPaths=rp.map(chain=>path(chain.map(map))).filter(Boolean),e=[[0,1],[1,2],[2,3],[3,0],[4,5],[5,6],[6,7],[7,4],[0,4],[1,5],[2,6],[3,7]];
    return{triangles,edges,slicePaths:roughPaths,sliceCount:modelSlices.length,chainCount:roughPaths.length,facePickingAvailable:faceIds.length===Math.floor(part.length/3),plane:path(fpl,true),stock:e.map(([i,j])=>path([fb[i],fb[j]])),axes:[path([fa[0],fa[1]]),path([fa[2],fa[3]]),path([fa[4],fa[5]])],labels:[fa[1],fa[3],fa[5]],wcs:fw};
  }
