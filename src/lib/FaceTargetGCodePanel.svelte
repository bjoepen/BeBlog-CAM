<script lang="ts">
  ...
  $: rawCode=postFaceTargetCanonicalToolpath(toolpath,{
  ...
  $: levels=[...new Set(toolpath.runs.map(run=>run.z.toFixed(6)))];
  $: valid=toolpath.operationKind==='z-level-roughing'&&toolpath.runs.length>0&&processed.ok;
...
{#if valid}
  ...
  <div class="export-box">
    <div><strong>NC-Programm Face-Target Roughing</strong><span>Explizite G0/G1-Bewegungen aus demselben kanonischen Toolpath wie die STEP-Vorschau.</span></div>
