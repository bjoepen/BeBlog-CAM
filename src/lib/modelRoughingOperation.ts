...
export function buildModelRoughingOperationState(args:{summary:ImportSummary;stock:StockDefinition;placement:PartPlacement;orientation:PartOrientation;wcs:WorkCoordinateSystem;operation:ZLevelRoughingOperation;profile?:ZLevelPerformanceProfile;}):ModelRoughingOperationState{
  ...
  const model=buildModelSliceRegions(zs.map(z=>sliceTrianglesAtZ(part,z,profile))),rough=buildRoughingRegions(model,{minX:0,minY:0,maxX:stock.width,maxY:stock.height}),invalid=rough.filter(r=>!r.valid);
  ...
  const o=origin(stock,wcs),built=buildModelRoughingCanonicalToolpath(rough,operation.tool.diameterMm,operation.stepoverPercent,o,profile);warnings.push(...built.warnings);errors.push(...built.errors);
  ...
  errors.push(...accessErrors(built.toolpath,rough,o,operation.tool.diameterMm,profile));return{ok:errors.length===0,toolpath:errors.length?null:built.toolpath,levelCount:zs.length,roughingRegionCount:rough.length,errors:[...new Set(errors)],warnings:[...new Set(warnings)]};
}
