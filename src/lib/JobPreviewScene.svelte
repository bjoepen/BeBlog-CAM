<script lang="ts">
  ...
  import { buildJobPreviewOperationStops, buildJobPreviewSimulationTimeline, sampleJobPreviewSimulation } from './jobPreviewSimulation';
  ...
  $: operationStops=buildJobPreviewOperationStops(timeline);
  ...
  $: currentOperationId=simulationFrame.currentStep?.segment.operationId??'';
  $: currentOperation=simulationFrame.currentStep?.segment.operationLabel??'—';
  $: currentMotionNumber=simulationFrame.currentStep?simulationFrame.completedSegmentCount+1:0;
  $: jobDistanceLabel=`${simulationDistance.toFixed(1)} / ${timeline.totalDistance.toFixed(1)} mm`;
  ...
  function pointerDown(event:PointerEvent){if(event.button!==0&&event.button!==1)return;const target=event.target as Element|null;if(target?.closest?.('button,select,input'))return;event.preventDefault();dragging=true;lastX=event.clientX;lastY=event.clientY;dragMode=event.shiftKey||event.button===1?'pan':'orbit';}
  ...
  function seekToDistance(distance:number){pauseSimulation();simulationDistance=Math.max(0,Math.min(timeline.totalDistance,distance));}
  function scrubSimulation(event:Event){seekToDistance(Number((event.currentTarget as HTMLInputElement).value)||0);}
  function jumpRelative(fraction:number){const delta=Math.max(timeline.totalDistance*.02,5)*fraction;seekToDistance(simulationDistance+delta);}
  function jumpToOperation(operationId:string){const stop=operationStops.find(item=>item.operationId===operationId);if(stop)seekToDistance(stop.startDistance);}
  function changeOperation(event:Event){jumpToOperation((event.currentTarget as HTMLSelectElement).value);}
  function jumpOperation(direction:-1|1){
    if(!operationStops.length)return;
    const currentIndex=Math.max(0,operationStops.findIndex(item=>item.operationId===currentOperationId));
    const nextIndex=Math.max(0,Math.min(operationStops.length-1,currentIndex+direction));
    jumpToOperation(operationStops[nextIndex].operationId);
  }
...
<div class="backdrop" role="presentation" onclick={(event)=>event.currentTarget===event.target&&onClose()}>
  <div class="dialog" role="dialog" aria-modal="true" aria-label="Gesamtjob Simulation Inspector">
    <header><div><p class="eyebrow">005C · Job Simulation Inspector</p><h2>Gesamtjob</h2><p class="subtitle">Gezielte Inspektion der bereits materialisierten Preflight-Motions. Read-only, ohne CAM-Neuberechnung.</p></div><div class="actions"><button onclick={resetView}>Ansicht zurücksetzen</button><button class="close" onclick={onClose}>Schließen</button></div></header>
    <div class="simulation-bar" aria-label="Simulationssteuerung">
      ...
      <button onclick={()=>jumpRelative(-1)} disabled={!scene.motionCount} aria-label="Im Job zurückspringen">− Schritt</button>
      <button onclick={()=>jumpRelative(1)} disabled={!scene.motionCount} aria-label="Im Job vorspringen">+ Schritt</button>
    ...
    <div class="inspector-bar" aria-label="Job Inspector Navigation">
      <button onclick={()=>jumpOperation(-1)} disabled={operationStops.length<2}>‹ Operation</button>
      <label>Operation
        <select value={currentOperationId} onchange={changeOperation} disabled={!operationStops.length}>
          {#each operationStops as stop}<option value={stop.operationId}>{stop.operationIndex+1}. {stop.operationLabel} · {stop.motionCount} Motions</option>{/each}
        </select>
      </label>
      <button onclick={()=>jumpOperation(1)} disabled={operationStops.length<2}>Operation ›</button>
      <div class="scrubber-wrap">
        <input class="scrubber" type="range" min="0" max={timeline.totalDistance||0} step={Math.max(timeline.totalDistance/2000,.01)} value={simulationDistance} oninput={scrubSimulation} aria-label="Gesamtjob Position" />
        <div class="scrubber-meta"><span>{(simulationFrame.progress*100).toFixed(1)} %</span><span>{jobDistanceLabel}</span><span>Motion {Math.min(currentMotionNumber,scene.motionCount)} / {scene.motionCount}</span></div>
      </div>
    </div>
    <div class="scene-shell" bind:this={sceneShell}>
      {#if scene.motionCount}
        <svg viewBox={`0 0 ${width} ${height}`} aria-label="Rohling, Bauteil und inspizierte Werkzeugbewegung des Gesamtjobs"><rect x="0" y="0" width={width} height={height} class="background"/>
