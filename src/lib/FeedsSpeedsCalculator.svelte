<script lang="ts">
  ...
  $: effectiveChipLoad=chipLoad*materialProfile.chipLoadFactor;
  $: valid=[diameter,cuttingSpeed,flutes,effectiveChipLoad,maxSpindleRpm,maxFeedMmMin].every(v=>Number.isFinite(v)&&v>0);
  $: calculated=valid?calculateFeedsSpeeds({toolDiameterMm:diameter,cuttingSpeedMMin:cuttingSpeed,flutes,chipLoadMm:effectiveChipLoad}):null;
  $: recommendedRpm=calculated?Math.min(calculated.spindleRpm,maxSpindleRpm):null;
  $: feedAtRecommendedRpm=recommendedRpm==null?null:recommendedRpm*flutes*effectiveChipLoad;
  ...
  function applyToOperation(){
    ...
    transferMessage=`Übernommen in ${activeOperationName}: ${materialProfile.label} · Ø ${n(diameter,2)} mm · ${n(Math.round(recommendedRpm))} 1/min · ${n(Math.round(recommendedFeed))} mm/min.`;
  }
</script>
...
<div class="tools-page">
  ...
  <div class="content-grid">
    <main class="main-column">
      ...
      <div class="two-cards">
        ...
        <section class="field-card"><div class="field-block"><b>Zahnvorschub (fz)</b><span class="input-line"><input class="numeric-input" type="number" min=".001" step=".005" bind:value={chipLoad}/><em>mm</em></span><small>Werkzeug-Basiswert; der Werkstofffaktor wird erst im Rechner angewendet</small></div></section>
      </div>
      ...
      {#if formulaOpen}<section class="formula"><code>n = (vc × 1000) / (π × d)</code><code>fz_material = fz_werkzeug × Materialfaktor</code><code>vf = n × z × fz_material</code><p>Die vollständige Berechnung und Maschinenbegrenzung liegt im Tab „Drehzahl &amp; Vorschub“.</p></section>{/if}
    </main>
    ...
    <aside class="side-column">
      ...
      <section class="card"><h3>Nächster Schritt</h3><p class="body-copy">Werkzeuggeometrie speichern oder direkt zu <b>Drehzahl &amp; Vorschub</b> wechseln. Dort werden Werkstoff, Schnittgeschwindigkeit und Maschinenlimits transparent zusammengeführt.</p><button class="wide-button" onclick={()=>activeTab='calculator'}>Drehzahl &amp; Vorschub öffnen →</button></section>
      <section class="hint"><b>ⓘ &nbsp; WERKZEUGBIBLIOTHEK</b><p>Gespeichert werden ausschließlich Werkzeugdaten. Der Werkstoff gehört zum Rohling und bleibt bewusst vom Werkzeug getrennt.</p></section>
    </aside>
  </div>
  ...
  <div class="content-grid">
    <main class="main-column">
      <header class="page-title"><span class="calc-icon">∑</span><div><h1>Drehzahl &amp; Vorschub</h1><p>Werkstoff, Werkzeug und reale Maschinenlimits getrennt und nachvollziehbar betrachten.</p></div></header>
      <p class="section-label">ZERSPANUNGSDATEN · {materialProfile.label.toUpperCase()}</p>
      <div class="two-cards">
        <section class="field-card"><div class="field-block"><b>Schnittgeschwindigkeit (vc)</b><span class="input-line"><input class="numeric-input" type="number" min="1" step="5" bind:value={cuttingSpeed}/><em>m/min</em></span><small>Startwert aus {materialProfile.label}; kann bewusst überschrieben werden</small></div></section>
        <section class="field-card"><div class="field-block"><b>Zahnvorschub Werkzeug (fz)</b><span class="input-line"><input class="numeric-input" type="number" min=".001" step=".005" bind:value={chipLoad}/><em>mm</em></span><small>Wirksam für {materialProfile.label}: {n(effectiveChipLoad,3)} mm/Z</small></div></section>
      </div>
      ...
      <button class="formula-toggle" onclick={()=>formulaOpen=!formulaOpen}><span>☷ &nbsp; Formeln &amp; Erklärung</span><b>{formulaOpen?'⌃':'⌄'}</b></button>
      {#if formulaOpen}<section class="formula"><code>n = (vc × 1000) / (π × d)</code><code>fz_material = fz_werkzeug × {n(materialProfile.chipLoadFactor,2)}</code><code>vf = n × z × fz_material</code><code>vf_begrenzt = n_begrenzt × z × fz_material</code><p>Wird die Drehzahl begrenzt, bleibt die werkstoffbezogene Zahnlast erhalten; der Vorschub wird passend zur realen Spindeldrehzahl neu berechnet.</p></section>{/if}
    </main>
    <aside class="side-column">
      ...
      <section class="card"><h3>Aktives Werkzeug</h3><div class="kv"><span>Name</span><b>{toolName}</b></div><div class="kv"><span>Durchmesser</span><b>{n(diameter,2)} mm</b></div><div class="kv"><span>Schneiden</span><b>{flutes}</b></div><div class="kv"><span>fz Werkzeug</span><b>{n(chipLoad,3)} mm</b></div><div class="kv"><span>fz wirksam</span><b>{n(effectiveChipLoad,3)} mm</b></div></section>
      <section class="apply-card"><div><h3>In aktive Bearbeitung übernehmen</h3><p>Ziel: <b>{activeOperationName}</b></p><p>Übernommen werden Werkzeug, Durchmesser sowie die aus <b>{materialProfile.label}</b> berechnete Drehzahl und der Vorschub. Tiefe, Zustellung, Eintauchvorschub und Sicherheits-Z bleiben unverändert.</p></div><button class="primary-action apply-button" disabled={!onApplyToOperation||recommendedRpm==null||recommendedFeed==null} onclick={applyToOperation}>Werkzeug &amp; Schnittdaten übernehmen</button>{#if transferMessage}<p class="transfer-message">✓ {transferMessage}</p>{/if}</section>
      <section class="card"><h3>Werkstoff des Rohlings</h3><div class="material-grid">{#each materialKinds as material}<button class:active={$stockMaterial===material} aria-pressed={$stockMaterial===material} onclick={()=>chooseMaterial(material)}>{materialProfiles[material].shortLabel}</button>{/each}</div><div class="material-summary"><b>{materialProfile.label}</b><span>vc {n(materialProfile.cuttingSpeedMMin)} m/min · fz-Faktor {n(materialProfile.chipLoadFactor,2)}</span></div><small>{materialProfile.note} Die 001S-Werte sind Startprofile und bleiben manuell anpassbar.</small></section>
      <section class="hint"><b>ⓘ &nbsp; TRANSPARENZ</b><p>Der Werkstoff beeinflusst sichtbar vc und den wirksamen Zahnvorschub. Ein Materialwechsel verändert noch keine CAM-Bearbeitung. Erst der explizite Übernehmen-Button schreibt die berechneten Werte in die aktive Operation.</p></section>
    </aside>
  </div>
  ...
  <div class="library-page">
    ...
    <section class="hint library-hint"><b>Gate 11C · Bibliothek</b><p>Werkzeugdaten werden persistent gespeichert. Werkstoffdaten gehören zum Rohling und werden nicht in Bibliothekswerkzeuge geschrieben. Die Übergabe in eine CAM-Bearbeitung erfolgt ausschließlich über „Werkzeug &amp; Schnittdaten übernehmen“.</p></section>
  </div>
  {/if}
</div>
