<script lang="ts">
  ...
  function saveTool(){
    ...
    selectedToolId=id;libraryMessage=exists?'Werkzeug aktualisiert.':'Werkzeug gespeichert.';
  }
  function loadTool(tool:LibraryTool){
    selectedToolId=tool.id;toolName=tool.name;diameter=tool.diameterMm;flutes=tool.flutes;chipLoad=tool.chipLoadMm;libraryMessage=`${tool.name} in Werkzeugdaten geladen.`;activeTab='data';
  }
  function editTool(tool:LibraryTool){selectedToolId=tool.id;toolName=tool.name;diameter=tool.diameterMm;flutes=tool.flutes;chipLoad=tool.chipLoadMm;activeTab='data';libraryMessage='Werkzeug zum Bearbeiten geladen.'}
  function deleteTool(id:string){persist(library.filter(tool=>tool.id!==id));if(selectedToolId===id)selectedToolId=null;libraryMessage='Werkzeug entfernt.'}
  function newTool(){selectedToolId=null;toolName='Neues Werkzeug';diameter=3;flutes=2;chipLoad=.03;activeTab='data';libraryMessage=''}
</script>
...
<div class="tools-page">
  ...
  <div class="content-grid">
    <main class="main-column">
      ...
      <section class="name-card"><label><b>Werkzeugname</b><input type="text" bind:value={toolName}/></label><button class="quiet" onclick={newTool}>Neu</button><button class="primary-action" onclick={saveTool}>{selectedToolId?'Aktualisieren':'In Bibliothek speichern'}</button></section>
  ...
  <div class="content-grid">
    <main class="main-column">
      ...
      <div class="two-cards">
        <section class="field-card"><label><b>Schnittgeschwindigkeit (vc)</b><span class="input-line"><input type="number" min="1" step="5" bind:value={cuttingSpeed}/><em>m/min</em></span><small>Herstellerangabe oder bewährter Richtwert</small></label></section>
        <section class="field-card"><label><b>Zahnvorschub (fz)</b><span class="input-line"><input type="number" min=".001" step=".005" bind:value={chipLoad}/><em>mm</em></span><small>Wird aus Werkzeugdaten übernommen</small></label></section>
      </div>
    ...
    <aside class="side-column">
      ...
      <section class="hint"><b>ⓘ &nbsp; TRANSPARENZ</b><p>BeBlog CAM zeigt rechnerischen Wert und Maschinenbegrenzung getrennt. Es wird kein versteckter „Hobby-CNC-Faktor“ angewendet.</p></section>
    </aside>
  </div>
  ...
  <div class="library-page">
    ...
    <section class="hint library-hint"><b>Gate 11C · Bibliothek</b><p>Werkzeugdaten werden persistent gespeichert. Die explizite Übergabe eines ausgewählten Werkzeugs samt bestätigter Schnittdaten in die aktive CAM-Bearbeitung folgt als nächster, getrennt testbarer Schritt.</p></section>
  </div>
  {/if}
</div>
