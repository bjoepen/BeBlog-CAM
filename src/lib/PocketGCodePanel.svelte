<script lang="ts">
  ...
  export let operation:PocketOperation;
  let copied=false;
...
{#if result.ok}
  ...
  <pre>{result.code}</pre>
{:else}
  <div class="release fail"><strong>FAIL</strong><span>Taschen-G-Code wird nur erzeugt, wenn die aktuelle Operation freigabefähig ist.</span></div>
  {#each result.errors as error}<p class="error-line"><strong>FAIL</strong> {error}</p>{/each}
  {#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
<p class="note"><strong>001H Gate 4:</strong> Nur die in Gate 3 freigegebene Rechtecktasche mit senkrechtem Eintauchen wird ausgegeben. Rampen bleiben gesperrt.</p>
