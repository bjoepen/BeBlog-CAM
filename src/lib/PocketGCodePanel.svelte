...
{#if result.ok}
  ...
  <div class="export-box"><div><strong>NC-Datei</strong><span>Speichert den geprüften Taschen-G-Code mit dem gewählten Postprozessor als .nc.</span></div><button class="primary-export" disabled={!processed.ok} onclick={saveNc}>G-Code speichern …</button></div>
...
{/if}
<p class="note"><strong>001K Kommentar-Polish:</strong> Operation, Strategie und besondere Bahnmerkmale werden im exportierten G-Code konsistent benannt.</p>
{/if}
