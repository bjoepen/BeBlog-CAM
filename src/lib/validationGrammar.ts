...
export function validateCommonOperation(operation: CamOperation, stock: StockDefinition, stockMode: StockMode, wcs: WorkCoordinateSystem): ValidationCheck[] {
  ...
  if(operation.kind==='z-level-roughing'){
    ...
    checks.push({level:'pass',category:'depth',title:'3D Schruppen',detail:'Zielfläche und Schlichtaufmaß werden aus einer eigenen 3D-Surface-Truth abgeleitet; der A1-Kernel erzeugt noch keinen Werkzeugweg.'});
  ...
  if(stockMode==='none'){
    const requiresStock=operation.kind==='facing'||operation.kind==='z-level-roughing'||operation.kind==='3d-roughing'||operation.kind==='surface-finishing'||(operation.kind==='contour'&&(operation.depthMode??'manual')==='stock-bottom');
