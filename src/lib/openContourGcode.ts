...
export type OpenContourGcodeResult={ok:boolean;errors:string[];warnings:string[];code:string;lineCount:number;pointCount:number;passes:number;radiusMm:number;interpolation:'g1-segmented';nativeArcCount:number;validation?:ReturnType<typeof offsetOpenChain>['validation']};
...
export function generateOpenContourGcode(args:{summary:ImportSummary;stock:StockDefinition;stockMode:StockMode;placement:PartPlacement;orientation:PartOrientation;wcs:WorkCoordinateSystem;operation:ContourOperation}):OpenContourGcodeResult{
  const {summary,stock,stockMode,placement,orientation,wcs,operation}=args;const errors:string[]=[],warnings:string[]=[];
  if(summary.kind!=='dxf')errors.push('Offene Konturen sind zunächst nur aus DXF freigegeben.');
  if(operation.topology!=='open')errors.push('Die Bearbeitung ist nicht als offene Kontur markiert.');
  if(operation.contourId===null)errors.push('Keine offene Kontur gewählt.');
  ...
  if(wcs.z!=='top')errors.push('Offene 2D-Konturen sind nur mit Z-Null auf der Rohlingoberseite freigegeben.');
  if(stockMode==='none')warnings.push('Kein Rohling definiert: Material- und Kollisionsgrenzen sind nicht vollständig prüfbar.');
  const fail=(validation?:ReturnType<typeof offsetOpenChain>['validation']):OpenContourGcodeResult=>({ok:false,errors,warnings,code:'',lineCount:0,pointCount:0,passes:0,radiusMm:operation.tool.diameterMm/2,interpolation:'g1-segmented',nativeArcCount:0,validation});
  ...
  const chains=buildOpenChains(summary.planarGeometry?.curves??[],move);const selected=operation.contourId===null?null:chains.find(c=>c.id===operation.contourId)??null;
  if(!selected){errors.push('Gewählte offene Kontur wurde in der aktuellen Geometrie nicht gefunden.');return fail();}
  const radius=operation.tool.diameterMm/2,offset=offsetOpenChain(selected.points,radius,operation.openSide,.003);
  if(!offset.validation.ok){
    if(offset.validation.selfIntersects)errors.push('Die radiuskorrigierte offene Werkzeugbahn schneidet sich selbst.');
    else errors.push(`Offene Werkzeugbahn hat die geometrische Prüfung nicht bestanden (max. Abweichung ${Number.isFinite(offset.validation.maxDeviationMm)?offset.validation.maxDeviationMm.toFixed(4):'—'} mm).`);
    return fail(offset.validation);
  }
  const origin=wcsOrigin(stock,stockMode,wcs,t.partBounds);let toolpath=offset.points.map(p=>({x:p.x-origin.x,y:p.y-origin.y}));
  ...
  const sideLabel=operation.openSide==='left'?'links':operation.openSide==='right'?'rechts':'auf Linie';
  lines.push('( BeBlog CAM 001Y )','( Operation: offene Kontur )',`( Werkzeugseite: ${sideLabel} relativ zur DXF-Kettenrichtung )`,`( Werkzeug Ø${f3(operation.tool.diameterMm)} mm · offene Endpunkte bleiben offen )`,'G21','G90','G17',`S${Math.round(operation.spindleRpm)} M3`,`G0 Z${f3(operation.safeZMm)}`);
  const start=toolpath[0];
  for(let pass=1;pass<=passes;pass++){
    const depth=-Math.min(operation.totalDepthMm,pass*operation.stepDownMm);
    lines.push(`( Zustellung ${pass}/${passes} · Z${f3(depth)} )`,`G0 X${f3(start.x)} Y${f3(start.y)}`,`G1 Z${f3(depth)} F${Math.round(operation.plungeMmMin)}`);
  ...
  lines.push('M5','M30');
  return{ok:true,errors:[],warnings,code:lines.join('\n')+'\n',lineCount:lines.length,pointCount:toolpath.length,passes,radiusMm:radius,interpolation:'g1-segmented',nativeArcCount:0,validation:offset.validation};
}
