...
<div class="overall" class:pass={level==='pass'} class:warn={level==='warn'} class:fail={level==='fail'}><strong>{level.toUpperCase()}</strong><span>{level==='pass'?'Planoperation ist für die aktuellen Prüfregeln freigegeben.':level==='warn'?'Planoperation ist freigabefähig, enthält aber Hinweise.':'Planoperation ist nicht freigegeben.'}</span></div>
...
<div class="check" class:pass={compatibility.level==='pass'} class:warn={compatibility.level==='warn'} class:fail={compatibility.level==='fail'}><strong>{compatibility.level.toUpperCase()} · Werkzeug</strong><p>{operation.tool.name} · Ø {n(operation.tool.diameterMm)} mm. {compatibility.detail}</p></div>
<div class="check" class:pass={operation.feedMmMin>0&&operation.plungeMmMin>0&&operation.spindleRpm>0&&operation.safeZMm>0} class:fail={!(operation.feedMmMin>0&&operation.plungeMmMin>0&&operation.spindleRpm>0&&operation.safeZMm>0)}><strong>{operation.feedMmMin>0&&operation.plungeMmMin>0&&operation.spindleRpm>0&&operation.safeZMm>0?'PASS':'FAIL'} · Schnittdaten</strong><p>{operation.feedMmMin} mm/min · Eintauchen {operation.plungeMmMin} mm/min · {operation.spindleRpm} 1/min · Sicherheits-Z {n(operation.safeZMm)} mm.</p></div>
...
<p class="note"><strong>Prüfregel:</strong> Planen bearbeitet die komplette rechteckige Rohlingoberseite. Planfräser sind bevorzugt; Schaftfräser bleiben als praktische Alternative zulässig.</p>
