...
<div class="facts"><span>{result.holeCount} Bohrposition{result.holeCount===1?'':'en'}</span><span>{result.passesPerHole} Zustellung{result.passesPerHole===1?'':'en'} je Bohrung</span><span>{displayCode.trimEnd().split(/\r?\n/).length} G-Code-Zeilen</span></div>
<p class="note"><strong>Strategie:</strong> Safe-Z → XY zur Bohrposition → G1-Zustellung → Zwischenrückzug bei mehreren Zustellungen → Safe-Z.</p>
<PostProcessorPicker/>
...
<div class="export-box"><div><strong>NC-Datei</strong><span>Speichert den geprüften Bohr-G-Code mit dem gewählten Postprozessor als .nc.</span></div><button class="primary-export" disabled={!processed.ok} onclick={saveNc}>G-Code speichern …</button></div>
