<script lang="ts">
  ...
  function defaultNcName(){const base=(summary.fileName||'beblog-cam').replace(/\.[^.]+$/,'').replace(/[^a-zA-Z0-9äöüÄÖÜß._ -]+/g,'-').trim()||'beblog-cam';return `${base}-bohren.nc`;}
...
{:else}
<p class="eyebrow">06 · Fräsen</p><h2>Bohren</h2>
{#if result.ok}
<div class="release pass"><strong>PASS</strong><span>Die Bohrpositionen sind geprüft und werden als explizite G0/G1-Bewegungen ausgegeben.</span></div>
<div class="facts"><span>{result.holeCount} Bohrposition{result.holeCount===1?'':'en'}</span><span>{result.passesPerHole} Zustellung{result.passesPerHole===1?'':'en'} je Bohrung</span><span>{result.lineCount} G-Code-Zeilen</span></div>
<p class="note"><strong>Strategie:</strong> Safe-Z → XY zur Bohrposition → G1-Zustellung → Zwischenrückzug bei mehreren Zustellungen → Safe-Z. Gate 9B verwendet bewusst keine controllerabhängigen G81/G83-Canned-Cycles.</p>
{#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
<div class="export-box"><div><strong>NC-Datei</strong><span>Speichert exakt den angezeigten Bohr-G-Code als .nc.</span></div><button class="primary-export" onclick={saveNc}>G-Code speichern …</button></div>
