...
export type StockSimulationResult={ok:boolean;errors:string[];warnings:string[];cellSizeMm:number;columns:number;rows:number;initialVolumeMm3:number;remainingVolumeMm3:number;removedVolumeMm3:number;removedPercent:number;minSurfaceZ:number;maxSurfaceZ:number;touchedCells:number;operationCount:number;heights:Float32Array;};
...
export function simulateStockHeightfield(args:{stock:StockDefinition;wcs:WorkCoordinateSystem;operations:StockSimulationOperation[];cellSizeMm?:number}):StockSimulationResult{
  const {stock,wcs,operations}=args,errors:string[]=[],warnings:string[]=[];
  ...
  if(wcs.z!=='top')errors.push('Stock-Simulation 004P ist nur mit Z-Null auf Rohlingoberseite freigegeben.');
  ...
  const columns=Math.max(1,Math.ceil(stock.width/requested)),rows=Math.max(1,Math.ceil(stock.height/requested)),dx=stock.width/columns,dy=stock.height/rows,cellArea=dx*dy,topZ=0,bottomZ=-stock.thickness;
  const heights=new Float32Array(columns*rows),touched=new Uint8Array(columns*rows);heights.fill(topZ);
  const cutDisk=(a:ToolpathPoint2,b:ToolpathPoint2,z:number,radius:number)=>{const target=clamp(z,bottomZ,topZ);const minX=Math.max(0,Math.floor((Math.min(a.x,b.x)-radius-xb.min)/dx)),maxX=Math.min(columns-1,Math.floor((Math.max(a.x,b.x)+radius-xb.min)/dx)),minY=Math.max(0,Math.floor((Math.min(a.y,b.y)-radius-yb.min)/dy)),maxY=Math.min(rows-1,Math.floor((Math.max(a.y,b.y)+radius-yb.min)/dy));for(let iy=minY;iy<=maxY;iy++)for(let ix=minX;ix<=maxX;ix++){const p={x:xb.min+(ix+.5)*dx,y:yb.min+(iy+.5)*dy};if(distanceToSegment(p,a,b)>radius)continue;const idx=iy*columns+ix,next=Math.min(heights[idx],target);if(next!==heights[idx]){heights[idx]=next;touched[idx]=1;}}};
  for(const entry of operations){const radius=entry.toolpath.tool.diameterMm/2;if(!(radius>0))continue;for(const run of entry.toolpath.runs){if(!run.points.length)continue;if(run.points.length===1)cutDisk(run.points[0],run.points[0],run.z,radius);else for(let i=1;i<run.points.length;i++)cutDisk(run.points[i-1],run.points[i],run.z,radius);}}
