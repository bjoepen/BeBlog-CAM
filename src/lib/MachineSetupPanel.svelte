<script lang="ts">
  ...
  export let origin:MachineWcsOrigin|undefined=undefined;
  ...
  $: effectiveOrigin=origin??wcsOrigin;
  const number=(event:Event)=>Number((event.currentTarget as HTMLInputElement).value);
  function emitEnabled(value:boolean){onEnabledChange(value);onChange?.({enabled:value,envelope,origin:effectiveOrigin});}
  function setEnvelope<K extends keyof MachineEnvelope>(key:K,event:Event){const value=number(event);if(Number.isFinite(value)){const next={...envelope,[key]:value};onEnvelopeChange(next);onChange?.({enabled,envelope:next,origin:effectiveOrigin});}}
  function setOrigin<K extends keyof MachineWcsOrigin>(key:K,event:Event){const value=number(event);if(Number.isFinite(value)){const next={...effectiveOrigin,[key]:value};onWcsOriginChange(next);onChange?.({enabled,envelope,origin:next});}}
</script>
<div class="machine-setup">
  <div class="head"><div><strong>Maschinenarbeitsraum · 004S</strong><span>Reale Achsgrenzen und aktuelle WCS-Position in Maschinenkoordinaten.</span></div><div class="toggle"><button class:active={!enabled} onclick={()=>emitEnabled(false)}>Aus</button><button class:active={enabled} onclick={()=>emitEnabled(true)}>Aktiv</button></div></div>
  {#if enabled}
    ...
    <div class="group"><strong>Aktueller WCS-Ursprung in Maschinenkoordinaten</strong><div class="grid three"><label>X <input type="number" step="0.1" value={effectiveOrigin.x} oninput={e=>setOrigin('x',e)}/> mm</label><label>Y <input type="number" step="0.1" value={effectiveOrigin.y} oninput={e=>setOrigin('y',e)}/> mm</label><label>Z <input type="number" step="0.1" value={effectiveOrigin.z} oninput={e=>setOrigin('z',e)}/> mm</label></div><p class="note">004S addiert diese Maschinenposition zu allen kanonischen WCS-Koordinaten. Erst danach werden X/Y/Z-Limits geprüft.</p></div>
