...
const tolerance=.08;
const semanticEps=1e-8;
const dist=(a:P2,b:P2)=>Math.hypot(a.x-b.x,a.y-b.y);
const same=(a:P2,b:P2)=>dist(a,b)<=tolerance;
const isDegenerateLine=(s:SemanticSegment)=>s.kind==='line'&&dist(s.start,s.end)<=semanticEps;

function bulgeSegment(a:P2,b:P2,bulge:number):SemanticSegment|null{
  const chord=dist(a,b);
  if(chord<=semanticEps)return null;
  if(Math.abs(bulge)<1e-12)return{kind:'line',start:a,end:b};
  const dx=b.x-a.x,dy=b.y-a.y;
  const mx=(a.x+b.x)/2,my=(a.y+b.y)/2,nx=-dy/chord,ny=dx/chord;
  const h=chord*(1-bulge*bulge)/(4*bulge),center={x:mx+nx*h,y:my+ny*h};
  return{kind:'arc',start:a,end:b,center,radius:chord*(1+bulge*bulge)/(4*Math.abs(bulge)),ccw:bulge>0};
}

function polylineSegments(c:Extract<Curve2,{kind:'polyline'}>,transform:(p:P2)=>P2=(p)=>p):SemanticSegment[]{
  const pts=c.points.map(transform),bulges=c.bulges??[],out:SemanticSegment[]=[];
  if(pts.length<2)return out;
  const count=c.closed?pts.length:pts.length-1;
  for(let i=0;i<count;i++){
    const seg=bulgeSegment(pts[i],pts[(i+1)%pts.length],bulges[i]??0);
    if(seg)out.push(seg);
  }
  return out;
}

function sampleSemanticSegment(s:SemanticSegment,steps=24):P2[]{
  ...
  return Array.from({length:steps+1},(_,i)=>{
    ...
    return{x:s.center.x+Math.cos(a)*s.radius,y:s.center.y+Math.sin(a)*s.radius};
  });
}
...
export function sampleCurve(c:Curve2):Point2[]{
  ...
  if(c.kind==='polyline'){
    const segs=polylineSegments(c),out:P2[]=[];
    for(const s of segs){const p=sampleSemanticSegment(s);out.push(...(out.length?p.slice(1):p))}
    return out;
  }
...
export function buildClosedChains(curves:Curve2[],transform:(p:P2)=>P2=(p)=>p):Chain[]{
  const closed:Chain[]=[],open:P2[][]=[];
  for(const c of curves){
    let pts=sampleCurve(c).map(transform);
...
export function offsetPolygon(input:P2[],distance:number):P2[]{
  ...
  let pts=[...input];
  if(same(pts[0],pts[pts.length-1]))pts=pts.slice(0,-1);
  if(pts.length<3)return input.map(p=>({...p}));
  const area=polygonArea(pts),outwardSign=area>0?-1:1,d=distance*outwardSign,result:P2[]=[];
...
export function validateOffsetSegments(sourceInput:P2[],toolInput:P2[],signedDistanceMm:number,toleranceMm=.002):OffsetValidation{
  ...
  if(source.length!==tool.length||source.length<3)return{ok:false,expectedMm:Math.abs(signedDistanceMm),measuredMinMm:NaN,measuredMaxMm:NaN,maxDeviationMm:Infinity,maxParallelError:Infinity,segmentCount:0,sideOk:false};
  const area=polygonArea(source),outwardSign=area>0?-1:1,expectedSigned=signedDistanceMm*outwardSign;
  let min=Infinity,max=-Infinity,maxDev=0,maxParallel=0,sideOk=true,count=0;
  for(let i=0;i<source.length;i++){
    const a=source[i],b=source[(i+1)%source.length],ta=tool[i],tb=tool[(i+1)%tool.length],sx=b.x-a.x,sy=b.y-a.y,sl=Math.hypot(sx,sy);
    if(sl<1e-8)continue;
    const tx=tb.x-ta.x,ty=tb.y-ta.y,tl=Math.hypot(tx,ty);if(tl<1e-8)continue;
    const su={x:sx/sl,y:sy/sl},tu={x:tx/tl,y:ty/tl},mid={x:(ta.x+tb.x)/2,y:(ta.y+tb.y)/2};
    const signed=su.x*(mid.y-a.y)-su.y*(mid.x-a.x),measured=Math.abs(signed),expected=Math.abs(signedDistanceMm);
  ...
  return{ok:count>0&&maxDev<=toleranceMm&&maxParallel<=1e-6&&sideOk,expectedMm:Math.abs(signedDistanceMm),measuredMinMm:min,measuredMaxMm:max,maxDeviationMm:maxDev,maxParallelError:maxParallel,segmentCount:count,sideOk};
}
...
function curveUnit(c:Curve2,transform:(p:P2)=>P2):SemanticSegment[]|null{
  if(c.kind==='line'){
    const start=transform(c.start),end=transform(c.end);
    return dist(start,end)<=semanticEps?[]:[{kind:'line',start,end}];
  }
  if(c.kind==='arc'){
    ...
    return c.radius<=semanticEps?[]:[{kind:'arc',start:s,end:e,center,radius:c.radius,ccw:true}];
  }
  if(c.kind==='polyline')return polylineSegments(c,transform);
  return null;
}

export function buildSemanticContours(curves:Curve2[],transform:(p:P2)=>P2=(p)=>p):SemanticContour[]{
  const closed:SemanticContour[]=[],open:{segments:SemanticSegment[];supported:boolean}[]=[];
  for(const c of curves){
    if(c.kind==='circle'){closed.push({id:closed.length,segments:[],supported:false});continue}
    const raw=curveUnit(c,transform);
    const segs=raw?.filter(s=>!isDegenerateLine(s))??raw;
    if(c.kind==='polyline'&&c.closed){
      if(segs?.length)closed.push({id:closed.length,segments:segs,supported:true});
      continue;
    }
  ...
  for(let seed=0;seed<open.length;seed++){
    if(used.has(seed))continue;
    used.add(seed);
    let segs=[...open[seed].segments],supported=open[seed].supported,progress=true;
    while(progress&&segs.length){
      progress=false;
      const a=segs[0].start,b=segs[segs.length-1].end;
      for(let i=0;i<open.length;i++){
        if(used.has(i)||!open[i].segments.length)continue;
        const s=open[i].segments,s0=s[0].start,s1=s[s.length-1].end;
        if(same(b,s0)){segs=[...segs,...s];used.add(i);progress=true;break}
        if(same(b,s1)){segs=[...segs,...reverseSegments(s)];used.add(i);progress=true;break}
        if(same(a,s1)){segs=[...s,...segs];used.add(i);progress=true;break}
        if(same(a,s0)){segs=[...reverseSegments(s),...segs];used.add(i);progress=true;break}
      }
    }
    segs=segs.filter(s=>!isDegenerateLine(s));
...
function lineCircle(line:SemanticSegment&{kind:'line'},arc:SemanticSegment&{kind:'arc'}):P2[]{
  if(dist(line.start,line.end)<=semanticEps)return[];
  const d=unitFrom(line.start,line.end),fx=line.start.x-arc.center.x,fy=line.start.y-arc.center.y;
  const b=2*(fx*d.x+fy*d.y),c=fx*fx+fy*fy-arc.radius*arc.radius,rawDisc=b*b-4*c;
  const eps=1e-10*Math.max(1,Math.abs(b*b),Math.abs(4*c),arc.radius*arc.radius);
  if(rawDisc < -eps)return[];
  const disc=Math.max(0,rawDisc),q=Math.sqrt(disc);
...
function circleCircle(a:SemanticSegment&{kind:'arc'},b:SemanticSegment&{kind:'arc'}):P2[]{
  ...
  if(d<1e-9||d>a.radius+b.radius||d<Math.abs(a.radius-b.radius))return[];
...
function sampledSemantic(segs:SemanticSegment[]):P2[]{const pts:P2[]=[];for(const s of segs){const p=sampleSemanticSegment(s);pts.push(...(pts.length?p.slice(1):p))}return pts}
...
export function offsetSemanticContour(contour:SemanticContour,correction:number,toleranceMm=.002):SemanticOffsetResult|null{
  const source=contour.segments.filter(s=>!isDegenerateLine(s));
  if(!contour.supported||source.length<2)return null;
  const sourcePts=sampledSemantic(source),area=polygonArea(sourcePts);
  if(Math.abs(area)<1e-9)return null;
  const outwardSign=area>0?-1:1,d=correction*outwardSign;
  let out:SemanticSegment[]=[];
  for(const s of source){
    if(s.kind==='line'){
      const u=unitFrom(s.start,s.end),n={x:-u.y*d,y:u.x*d};
      out.push({kind:'line',start:{x:s.start.x+n.x,y:s.start.y+n.y},end:{x:s.end.x+n.x,y:s.end.y+n.y}});
    }else{
      const r=s.radius-d*(s.ccw?1:-1);
      if(r<=0)return null;
      const us=unitFrom(s.center,s.start),ue=unitFrom(s.center,s.end);
      out.push({kind:'arc',center:{...s.center},radius:r,ccw:s.ccw,start:{x:s.center.x+us.x*r,y:s.center.y+us.y*r},end:{x:s.center.x+ue.x*r,y:s.center.y+ue.y*r}});
    }
  }

  for(let i=0;i<out.length;i++){
    const j=(i+1)%out.length,target=source[i].end,hit=joinPoint(out[i],out[j],target);
    if(!hit||dist(hit,target)>Math.max(25,Math.abs(correction)*10))return null;
    out[i]=setEnd(out[i],hit);out[j]=setStart(out[j],hit);
  }

  const expected=Math.abs(correction);
  let min=Infinity,max=-Infinity,maxDev=0,sideOk=true,minIndex=-1,worstIndex=-1;
  for(let i=0;i<out.length;i++){
    const src=source[i],tool=out[i];
    let measured=0;
    if(src.kind==='line'&&tool.kind==='line'){
      if(dist(tool.start,tool.end)<=semanticEps){
        return{segments:out,validation:{ok:false,expectedMm:expected,measuredMinMm:0,measuredMaxMm:expected,maxDeviationMm:expected,maxParallelError:0,segmentCount:out.length,sideOk:true,diagnostic:`Segment ${i+1} LINE kollabiert beim Trimmen der nativen Offsetkontur.`}};
      }
      const u=unitFrom(src.start,src.end),mid={x:(tool.start.x+tool.end.x)/2,y:(tool.start.y+tool.end.y)/2};
      const signed=u.x*(mid.y-src.start.y)-u.y*(mid.x-src.start.x);
      measured=Math.abs(signed);
      if(Math.abs(correction)>1e-9&&Math.sign(signed)!==Math.sign(d))sideOk=false;
    }else if(src.kind==='arc'&&tool.kind==='arc'){
      measured=Math.abs(tool.radius-src.radius);
      const expectedRadius=src.radius-d*(src.ccw?1:-1);
      if(Math.abs(tool.radius-expectedRadius)>toleranceMm)sideOk=false;
    }else return null;

    if(measured<min){min=measured;minIndex=i}
    max=Math.max(max,measured);
    const deviation=Math.abs(measured-expected);
    if(deviation>maxDev){maxDev=deviation;worstIndex=i}
  }
  const continuity=Math.max(...out.map((s,i)=>dist(s.end,out[(i+1)%out.length].start)));
  maxDev=Math.max(maxDev,continuity);
  const failingIndex=worstIndex>=0?worstIndex:minIndex;
  const diagnostic=maxDev>toleranceMm&&failingIndex>=0
    ?`Größte Abweichung an Segment ${failingIndex+1} (${source[failingIndex].kind.toUpperCase()}); gemessen ${min.toFixed(4)}–${max.toFixed(4)} mm.`
    :undefined;
  return{segments:out,validation:{ok:maxDev<=toleranceMm&&sideOk,expectedMm:expected,measuredMinMm:min,measuredMaxMm:max,maxDeviationMm:maxDev,maxParallelError:0,segmentCount:out.length,sideOk,diagnostic}};
}
