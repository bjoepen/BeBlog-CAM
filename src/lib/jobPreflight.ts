...
export function validateJob(args:{summary:ImportSummary;stock:StockDefinition;stockMode:StockMode;placement:PartPlacement;orientation:PartOrientation;wcs:WorkCoordinateSystem;operations:CamOperation[]}):JobPreflightResult{
  const {summary,stock,stockMode,placement,orientation,wcs}=args,enabled=args.operations.filter(op=>op.enabled!==false),operations:JobPreflightOperation[]=[],errors:string[]=[],warnings:string[]=[];
  enabled.forEach((operation,index)=>{
    ...
    if(operation.kind==='facing'){const r=generateFacingGcode({stock,stockMode,wcs,operation});opErrors=[...r.errors];opWarnings=[...r.warnings];detail=`${operation.direction==='x'?'X-Raster':'Y-Raster'} · Ø ${operation.tool.diameterMm.toFixed(3)} mm · ${operation.totalDepthMm.toFixed(3)} mm Abtrag`;if(r.ok)canonicalToolpath=buildFacingToolpath({stock,wcs,operation}).toolpath;}
      ...
      if(summary.kind==='step'){
        const state=buildStepDrillOperationState({summary,stock,stockMode,placement,orientation,wcs,operation});opErrors=[...state.errors];opWarnings=[...state.warnings];canonicalToolpath=state.toolpath;detail=`STEP BRep · Bohren · ${state.holes.length} Position${state.holes.length===1?'':'en'} · Werkzeug Ø ${operation.tool.diameterMm.toFixed(3)} mm · Tiefe aus Hole Feature`;
      }else{
        const r=validateDrillOperation(summary,operation);opErrors=[...r.errors];opWarnings=[...r.warnings];detail=`${operation.method==='helical-mill'?'Helixfräsen':'Bohren'} · ${r.holeCount} Position${r.holeCount===1?'':'en'} · Werkzeug Ø ${operation.tool.diameterMm.toFixed(3)} mm · ${operation.totalDepthMm.toFixed(3)} mm tief`;if(!opErrors.length)canonicalToolpath=buildDrillCanonicalToolpath({summary,stock,stockMode,placement,orientation,wcs,operation});
      }
    }
    else if(operation.kind==='pocket'){const r=generatePocketGcode({summary,stock,stockMode,placement,orientation,wcs,operation});opErrors=[...r.errors];opWarnings=[...r.warnings];detail=`Tasche · Ø ${operation.tool.diameterMm.toFixed(3)} mm · ${operation.totalDepthMm.toFixed(3)} mm tief`;if(r.ok)canonicalToolpath=buildPocketCanonicalToolpath({summary,stock,stockMode,placement,orientation,wcs,operation});}
    ...
    opErrors=unique(opErrors);opWarnings=unique(opWarnings);const level:JobPreflightLevel=opErrors.length?'fail':opWarnings.length?'warn':'pass',label=operation.name||kindLabel(operation.kind);operations.push({id:operation.id,index:index+1,kind:operation.kind,label,detail,canonical:canonical.summary,level,errors:opErrors,warnings:opWarnings});errors.push(...opErrors.map(e=>`Bearbeitung ${index+1} (${kindLabel(operation.kind)}): ${e}`));warnings.push(...opWarnings.map(w=>`Bearbeitung ${index+1} (${kindLabel(operation.kind)}): ${w}`));
  });
