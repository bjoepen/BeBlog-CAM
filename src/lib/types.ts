...
export type Curve2 =
  ...
  | { kind: 'polyline'; points: Point2[]; bulges?: number[]; closed: boolean }
  | { kind: 'unsupported'; sourceKind: string };

export interface PlanarGeometry { curves: Curve2[]; bounds?: Bounds2; }
export interface SurfaceTypeSummary { kind: string; count: number; }
export interface BrepSummary { backend:string; nativeBrep:boolean; faces:number; edges:number; vertices:number; solids:number; surfaceTypes:SurfaceTypeSummary[]; cylinderRadiiMm:number[]; displayTriangles:number; displayVertices:number[]; note:string; }
...
export type HorizontalPlacement='left'|'center'|'right'; export type VerticalPlacement='front'|'center'|'back';
...
export type WcsX='left'|'center'|'right'; export type WcsY='front'|'center'|'back'; export type WcsZ='top'|'bottom'; export type WcsReference='stock'|'part';
