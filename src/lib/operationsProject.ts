...
export function operationSummary(operation:CamOperation):string {
  ...
  if(operation.kind==='z-level-roughing'){
    ...
    const raster=(operation.rasterDirection??'auto')==='auto'?'Raster Auto':operation.rasterDirection==='x'?'Raster X':'Raster Y';
    return `${source} · ${islands} · ${raster} · ${operation.stepDownMm.toLocaleString('de-DE',{maximumFractionDigits:3})} mm Zustellung · ${operation.stepoverPercent}% Stepover · ${operation.finishAllowanceMm.toLocaleString('de-DE',{maximumFractionDigits:3})} mm Aufmaß · ${tool}`;
  }
