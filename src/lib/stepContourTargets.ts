...
function sampleEdge(edge:StepManufacturingEdgeSource):P2[]|null{
  ...
  let a0=Math.atan2(start.y-center.y,start.x-center.x),a1=Math.atan2(end.y-center.y,start.x-center.x);
...
function chainWire(wire:StepManufacturingWireSource,edges:StepManufacturingEdgeSource[]):StepContourTargetSegment[]|null{
  ...
  const out:StepContourTargetSegment[]=[{edgeId:remaining[0].edgeId,points:[...remaining[0].points]}];
  remaining.shift();
...
export function buildStepContourTargets(summary:ImportSummary):StepContourTargetResult{
  ...
  for(const face of source.source.planarFaces){
    ...
    for(const wire of source.source.wiresByFace.get(face.faceId)??[]){
      ...
      const area=wire.closed&&points.length>=4?Math.abs(polygonArea(points)):null;if(area!==null&&area<=EPS)continue;
      targets.push({targetKey:`step-wire:${wire.wireId}`,wireId:wire.wireId,faceId:face.faceId,zMm:face.origin[2],topology:wire.closed?'closed':'open',edgeIds:segments.map(segment=>segment.edgeId),segments,points,areaMm2:area});
    }
  }
  targets.sort((a,b)=>b.zMm-a.zMm||(b.areaMm2??0)-(a.areaMm2??0)||a.wireId-b.wireId);
...
export function stepContourTargetAfterExclusions(target:StepContourTarget,excludedEdgeIds:number[]):StepContourTarget|null{
  ...
  const excluded=new Set(excludedEdgeIds),segments=target.segments.filter(segment=>!excluded.has(segment.edgeId));
  if(!segments.length)return null;
  const chains:StepContourTargetSegment[][]=[];let chain:StepContourTargetSegment[]=[];
  for(const segment of target.segments){
    if(excluded.has(segment.edgeId)){if(chain.length)chains.push(chain);chain=[];continue}
    chain.push(segment);
  }
  if(chain.length)chains.push(chain);
  if(target.topology==='closed'&&chains.length>1){const first=chains[0],last=chains[chains.length-1],lastEnd=last.at(-1)!.points.at(-1)!,firstStart=first[0].points[0];if(same(lastEnd,firstStart))chains.splice(0,1, [...last,...first]);}
  if(chains.length!==1)return null;
  const kept=chains[0],points=flattenSegments(kept,false);
  return{...target,targetKey:`${target.targetKey}:open:${kept.map(segment=>segment.edgeId).join(',')}`,topology:'open',edgeIds:kept.map(segment=>segment.edgeId),segments:kept,points,areaMm2:null};
}
