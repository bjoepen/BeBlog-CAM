...
export function generateClosedContourGcode(args:{summary:ImportSummary;stock:StockDefinition;stockMode:StockMode;placement:PartPlacement;orientation:PartOrientation;wcs:WorkCoordinateSystem;operation:ContourOperation}):GcodeResult{
  const {summary,stock,stockMode,placement,orientation,wcs,operation}=args;
  const errors:string[]=[],warnings:string[]=[];
  if(summary.kind!=='dxf')errors.push('Kontur-G-Code wird zunächst nur aus DXF-Konturen erzeugt.');
  ...
  if(wcs.z!=='top')errors.push('G-Code mit WCS auf Unterseite ist für 2D-Konturen noch gesperrt.');
  ...
  const lines:string[]=[];
  lines.push('( BeBlog CAM 001Y )');
  lines.push('( Operation: geschlossene Kontur )');
