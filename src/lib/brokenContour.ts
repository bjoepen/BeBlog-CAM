...
export function closedContourSegmentCount(points:P2[]):number{
  return basePoints(points).length;
}
...
export function buildActiveClosedRuns(points:P2[],excludedIds:number[]):P2[][]{
  ...
  for(let step=1;step<=n;step++){
    ...
    if(excluded.has(i)){
      if(current.length>1){runs.push(current);current=[];}
      continue;
    }
...
export function buildBrokenContourPath(points:P2[],excludedIds:number[],radiusMm:number,side:ToolpathSide,toleranceMm=.003):BrokenContourPath{
  ...
  for(const run of sourceRuns){
    ...
    maxDeviationMm=Math.max(maxDeviationMm,v.maxDeviationMm);
    maxParallelError=Math.max(maxParallelError,v.maxParallelError);
    selfIntersects=selfIntersects||v.selfIntersects;
