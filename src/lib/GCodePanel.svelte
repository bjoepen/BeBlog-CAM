<script lang="ts">
  ...
  $: interpolationLabel=result.interpolation==='g2g3-native-circle'?`Native Kreisinterpolation · ${result.nativeArcCount} G2/G3-Bögen insgesamt`:result.interpolation==='g1-g2g3-mixed'?`Gemischte native Kontur · G1-Linien + ${result.nativeArcCount} G2/G3-Bögen insgesamt`:'G1-Referenzbahn · native DXF-Semantik für diese Kontur nicht eindeutig freigabefähig';
...
{#if result.ok}
  ...
  <div class="facts"><span>{result.passes} Zustellung{result.passes===1?'':'en'}</span><span>{result.pointCount} Bahnprimitive</span><span>{displayCode.trimEnd().split(/\r?\n/).length} G-Code-Zeilen</span><span>Radius {result.radiusMm.toFixed(3)} mm</span></div>
  ...
  <div class="export-box"><div><strong>NC-Datei</strong><span>Speichert den geprüften Kontur-G-Code mit dem gewählten Postprozessor als .nc.</span></div><button class="primary-export" disabled={!processed.ok} onclick={saveNc}>G-Code speichern …</button></div>
...
{:else}
  <div class="release fail"><strong>FAIL</strong><span>G-Code wird nicht erzeugt, solange ein sicherheitsrelevanter Punkt offen ist.</span></div>{#each result.errors as error}<p class="error-line"><strong>FAIL</strong> {error}</p>{/each}{#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
{/if}
