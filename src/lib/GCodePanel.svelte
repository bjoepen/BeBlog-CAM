<script lang="ts">
  ...
  $: interpolationLabel=result.interpolation==='g2g3-native-circle'
    ?`Native Kreisinterpolation · ${result.nativeArcCount} G2/G3-Bögen insgesamt`
    :result.interpolation==='g1-g2g3-mixed'
      ?`Gemischte native Kontur · G1-Linien + ${result.nativeArcCount} G2/G3-Bögen insgesamt`
      :'G1-Referenzbahn · native DXF-Semantik für diese Kontur nicht eindeutig freigabefähig';
  ...
  async function saveNc(){
    ...
    try{
      ...
      await invoke('save_nc_file',{path,code:result.code});
      exportState='saved';
...
{#if result.ok}
  <div class="release pass"><strong>PASS</strong><span>Die aktuelle Kontur hat die geometrische Bahnprüfung bestanden und kann als G-Code ausgegeben werden.</span></div>
  <div class="facts"><span>{result.passes} Zustellung{result.passes===1?'':'en'}</span><span>{result.pointCount} Bahnprimitive</span><span>{result.lineCount} G-Code-Zeilen</span><span>Radius {result.radiusMm.toFixed(3)} mm</span></div>
  <p class="note"><strong>Interpolation:</strong> {interpolationLabel}.</p>
  ...
  <div class="export-box">
    <div><strong>NC-Datei</strong><span>Speichert exakt den aktuell geprüften G-Code als .nc.</span></div>
    <button class="primary-export" onclick={saveNc}>G-Code speichern …</button>
  </div>
  ...
  <div class="code-head"><span>Vorschau · identisch zur .nc-Datei</span><button onclick={copyCode}>{copied?'Kopiert':'G-Code kopieren'}</button></div>
  <pre>{result.code}</pre>
{:else}
  ...
  {#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
<p class="note"><strong>001G-Regel:</strong> Native Kreis-/Bogeninterpolation wird nur dann ausgegeben, wenn die zugrunde liegende CAD-Semantik eindeutig erhalten und die analytische Offsetbahn geprüft ist. Keine nachträgliche Bogen-Erkennung aus einer segmentierten Polylinie.</p>
