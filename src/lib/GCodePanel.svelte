...
{:else}
  ...
  {#each result.warnings as warning}<p class="warning"><strong>Hinweis:</strong> {warning}</p>{/each}
{/if}
<p class="note"><strong>001F-Regel:</strong> Die G-Code-Koordinaten werden aus der verifizierten Werkzeugmittelbahn relativ zum gewählten WCS abgeleitet. Die CAD-Kontur bleibt das Sollmaß.</p>
